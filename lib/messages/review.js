...
class Review extends Message {
  constructor({
    review, pull_request, repository, sender, slackUser
  }) {
    super({
  ...
  toJSON() {
    ...
    let pretext = `${this.sender.login} ${this.middle} pull request`;

    if (this.slackUser){
      pretext = `<@${this.slackUser.slackId}> ${this.middle} pull request`;
    }
